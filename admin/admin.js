...
const schemas = {
  ...
  featureFlags: [["title", "Configuration", "text", true], ["adsRuntimeEnabled", "Ads runtime enabled", "checkbox"], ["intrusionEnabled", "Intrusion enabled", "checkbox"], ["galleryStudioEnabled", "Gallery Studio enabled", "checkbox"], ["upgradedMediaPlayerEnabled", "Upgraded Media Player enabled", "checkbox"]],
...
function render() {
  ...
  (content[section] || []).forEach((entry, index) => {
    ...
    button.innerHTML = `<strong>${escapeHtml(entry.publicTitle || entry.displayName || entry.name || entry.label || entry.title || entry.predicate || "Untitled")}</strong><small>${escapeHtml(entry.status || entry.publicationState || entry.sourceType || entry.slug || entry.applicationId || entry.url || entry.color || "draft")}</small>`;

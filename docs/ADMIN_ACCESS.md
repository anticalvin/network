...
Use the hosted Supabase Dashboard or a server-only Admin API client to create the account. The preferred Dashboard flow is Authentication > Users > Add user > Send invitation, followed by setting the user's app metadata to `{ "user_role": "admin" }`. Never insert directly into `auth.users`, place a service-role key in this repository, or store the admin password in frontend configuration. After changing app metadata, the user must sign in again or refresh the session so the JWT contains the admin claim.
